...
def write_vtk(coord, num_ngd, tstp, elnames, phnames, mf, mur, phf):
    ...
    parts.append(s.getvalue()[:-2] + '\n')


    if n_dim == 1:
        nx, ny, nz = num_ngd[0], 1, 1
        parts.extend(['CELLS ', str(nx-1), ' ', str((nx-1)*(2**n_dim+1)), '\n'])
        x = np.arange(nx)
        n_dim_pow = 2**n_dim
        p0 = x[:-1]
        p1 = x[1:]
        ...
        parts.extend(['CELL_TYPES ', str(nx-1), '\n'])
        type_array = np.ones((1, (nx-1)), dtype=int)*4
        parts.extend([' '.join(map(str, type_array.ravel())), '\n'])
    if n_dim == 2:
        nx, ny, nz = num_ngd[0], num_ngd[1], 1
        parts.extend(['CELLS ', str((nx-1)*(ny-1)), ' ', str((nx-1)*(ny-1)*(2**n_dim+1)), '\n'])
        ...
        parts.append(cells_str)
        parts.extend(['CELL_TYPES ', str((nx-1)*(ny-1)), '\n'])
        type_array = np.ones((1, (nx-1)*(ny-1)), dtype=int)*8
        parts.extend([' '.join(map(str, type_array.ravel())), '\n'])
    parts.extend(['POINT_DATA ', str(np.prod(num_ngd)), '\n'])
